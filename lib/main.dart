...
class BookPage extends StatefulWidget {
  ...
  const BookPage({Key? key, required this.books, required this.category})
      : super(key: key);
...
class _BookPageState extends State<BookPage> {
  ...
  void _showDetailsOfBook(BuildContext context) {
...
List<Book> getBooksByCategory(String categoryId) {
  return DUMMY_BOOKS
      .where((book) => book.categories.contains(categoryId))
      .toList();
}
...
class _MyHomePageState extends State<MyHomePage> {
  @override
  Widget build(BuildContext context) {
    List<List<Category>> pairedCategories = [];
    for (var i = 0; i < DUMMY_CATEGORIES.length; i += 2) {
      if (i + 1 < DUMMY_CATEGORIES.length) {
        pairedCategories.add([
          DUMMY_CATEGORIES[i],
          DUMMY_CATEGORIES[i + 1],
        ]);
      } else {
        pairedCategories.add([DUMMY_CATEGORIES[i]]);
      }
    }
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return Row(
                    ...
                    children: pairedCategories[index].map((category) {
                      return GestureDetector(
                        onTap: () {
                          List<Book> filtered = getBooksByCategory(category.id);
                          Navigator.push(
                            context,
                            MaterialPageRoute(
